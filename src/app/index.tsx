...
export default function Index() {
  ...
  const [emailToInivite, setEmailToInvite] = useState("");
  const [emailsToInivite, setEmailsToInvite] = useState<string[]>([]);
  ...
  function handleRemoveEmail(emailToRemove: string) {
    setEmailsToInvite((prevState) =>
      prevState.filter((email: string) => email !== emailToRemove)
    );
  }

  function handleAddEmail() {
    if (!validateInput.email(emailToInivite)) {
      return Alert.alert("Convidado", "E-mail inválido!");
    }

    if (emailsToInivite.some((email: string) => email === emailToInivite)) {
      return Alert.alert("Convidado", "E-mail já foi adicionado!");
    }

    setEmailsToInvite((prevState) => [...prevState, emailToInivite]);
    setEmailToInvite("");
  }
  ...
  async function createTrip() {
    try {
      ...
      const newTrip = await tripServer.create({
        ...
        emails_to_invite: emailsToInivite,
      });
    ...
    <View className="flex-1 items-center justify-center px-5">
          ...
          <>
            ...
            <Input>
              ...
              <Input.Field
                ...
                autoCorrect={false}
                value={
                  emailsToInivite.length > 0
                    ? `${emailsToInivite.length} pessoas(a) convidada(s)`
                    : ""
                }
                onPressIn={() => {
                  Keyboard.dismiss();
      ...
      >
        <View className="my-2 flex-wrap gap-2 border-b border-zinc-800 py-5 items-start">
          {emailsToInivite.length > 0 ? (
            emailsToInivite.map((email: string) => (
              <GuestEmail
                email={email}
                key={email}
